...
inside the AccountingSaaS repository.
...
dotnet build AccountingSaaS.slnx

When tests are affected, also run:

dotnet test AccountingSaaS.slnx

When packages are added or updated, also run:

dotnet restore AccountingSaaS.slnx
dotnet list AccountingSaaS.slnx package --vulnerable --include-transitive
...
5. Do not log sensitive accounting data.
